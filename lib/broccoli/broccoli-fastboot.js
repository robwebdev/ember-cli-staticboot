...
function staticFilter(inputTree, options) {
  ...
  this.appendFileExtension = options.appendFileExtension;
...
staticFilter.prototype.outputPathForRoute = function (routePath, directory) {
  ...
  const outputPath = isIndex ? routePath + 'index' : routePath;
  const extension = this.appendFileExtension || isIndex ? '.html' : '';
  return `${path.join(directory, 'staticboot', outputPath)}${extension}`;
};
